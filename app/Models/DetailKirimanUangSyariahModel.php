...
class DetailKirimanUangSyariahModel extends Model
{
    ...
    public function getAllKirimanUangSyariah(){
        $query = $this->db->table('dbo_konten')
        ->where('konten_parent = 73')
        ->orderBy('konten_id', 'asc')
        ->get()->getResultArray();  
        return $query;
     }

        public function findDetailKirimanUangSyariahById($id)
    {
        $dbo_konten = $this
            ->asArray()
            ->where(['konten_id' => $id])
