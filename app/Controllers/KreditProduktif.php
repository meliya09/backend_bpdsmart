...
use App\Models\KreditProduktifModel;
...
class KreditProduktif extends BaseController 
{
    use ResponseTrait;
    // get all product
    public function index()
    {
        $model = new KreditProduktifModel();
        $data = $model->getAllKreditProduktif();
    ...
    public function show($konten_id)
    {
        try {
            $model = new KreditProduktifModel();
            $data = $model->findKreditProduktifById($konten_id);

            return $this->getResponse(
                [
                    'message' => 'Kredit Produktif retrieved successfully',
                    'produktif' => $data
                ]
            );
        } catch (Exception $e) {
