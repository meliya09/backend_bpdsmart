...
use App\Models\ DetailKreditStandarModel;
use CodeIgniter\RESTful\ResourceController;
use CodeIgniter\HTTP\ResponseInterface;
use CodeIgniter\API\ResponseTrait;
use Exception; 
 
class DetailKreditStandar extends ResourceController
{
	protected $modelName = 'App\Models\DetailKreditStandarModel';
	protected $format = 'json';
	use ResponseTrait;

	public function index(){
		$posts = $this->model->getAllKreditStandar();
		return $this->respond($posts);
	}

	public function show($konten_id = null){
		$data = $this->model->findDetailKreditStandarById($konten_id);
	...
	public function update4($id = null){
		...
		$fileName = dot_array_search('konten_promosi_gambar.name', $_FILES);

		if($fileName != ''){
			$images = ['konten_promosi_gambar' => 'uploaded[konten_promosi_gambar]|max_size[konten_promosi_gambar, 1024]|is_image[konten_promosi_gambar]'];
			$rules = array_merge($rules, $images);
		}
		if(!$this->validate($rules)){
			return $this->fail($this->validator->getErrors());
			...
			if($fileName != ''){
				...
				$file = $this->request->getFile('konten_promosi_gambar');
				$profile_image = $file->getName();
				...
				$data['konten_promosi_gambar'] = $file->getName();
				$data['file_path_promosi'] = "/images/".$file->getName();
	...
	public function approve($id = null){
		...
		$model = new DetailKreditStandarModel();
		$model->save($data);
		return $this->respond($data);
	}
