...
use App\Models\DetailLokasiATMModel;
use CodeIgniter\RESTful\ResourceController;
use CodeIgniter\HTTP\ResponseInterface;
use CodeIgniter\API\ResponseTrait;
use Exception;
 
class DetailLokasiATM extends ResourceController
{
	protected $modelName = 'App\Models\DetailLokasiATMModel';
	protected $format = 'json';
	use ResponseTrait;

	public function index(){
		$posts = $this->model->getAllDetailLokasiATM();
		return $this->respond($posts);
	}
 
	public function show($lokasi_parent = null){
		$data = $this->model->getByLokasiATMParent($lokasi_parent);
		return $this->respond($data);
	}

	public function showById($id = null){
		$data = $this->model->findDetailLokasiATMById($id);
		return $this->respond($data);
	}

	public function create($lokasi_parent = null)
    {
        $model = new DetailLokasiATMModel();
